...
def main(rank, metadata):
    dist.init_process_group(backend='nccl', world_size=metadata['num_gpu'], rank=rank)

    train_loader, train_len, test_loader, test_len, val_loader, val_len = Sound_Flip_distributed_data_loader(rank, metadata)

    model = Efficient(metadata['NameEfficient'], metadata['weightsPath'], N_CLASSES)
    ...
    for epoch in range(EPOCHS):
        ...
        train_distributed(model, CLASS_NAMES, N_CLASSES, epoch, train_loader, train_len, optimizer, writer, device, rank)
        model.eval()
        AUROC_avg, loss = val_distributed(model, CLASS_NAMES, N_CLASSES, epoch, val_loader, val_len, optimizer, writer, device, rank)
        model.train()
        scheduler.step(loss)
        if rank == 0:
    ...
    if rank == 0:
        ...
        model.eval()
        test(metadata, model, test_loader, test_len, CLASS_NAMES, N_CLASSES, writer)

#%%
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Get GPU Id')
    ...
    TensorDir,CheckPointDir = create_folders(name, rank)
