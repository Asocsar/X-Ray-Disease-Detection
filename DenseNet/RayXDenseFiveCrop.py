...
def main(metadata):
    ...
    train_loader, train_len, test_loader, test_len, val_loader, val_len = default_data_loader_FiveCrop(metadata)

    model = DenseNet121_Pretrained(N_CLASSES, metadata['weightsPath']).cuda()
    ...
    for epoch in range(EPOCHS):
        ...
        train(model, CLASS_NAMES, N_CLASSES, epoch, train_loader, train_len, optimizer, writer)
        model.eval()
        AUROC_avg, loss = val(model, CLASS_NAMES, N_CLASSES, epoch, val_loader, val_len, optimizer, writer)
        model.train()
        scheduler.step(loss)
