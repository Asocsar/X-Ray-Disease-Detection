...
def main(metadata):
    ...
    train_loader, train_len, test_loader, test_len, val_loader, val_len = RandomFlip_data_loader_NC(metadata)

    model = Efficient_Mixed(metadata['NameEfficient'], metadata['weightsPath'], N_CLASSES).cuda()
    ...
    for epoch in range(EPOCHS):
        ...
        train_NC_Mixed(model, CLASS_NAMES, N_CLASSES, epoch, train_loader, train_len, optimizer, writer)
        model.eval()
        AUROC_avg, loss = val_NC_Mixed(model, CLASS_NAMES, N_CLASSES, epoch, val_loader, val_len, optimizer, writer)
        model.train()
        scheduler.step(loss)
