...
def main(rank, metadata):
    ...
    train_loader, train_len, test_loader, test_len, val_loader, val_len = RandomFlip_data_loader_NC_distributed(rank, metadata)

    model = Efficient_Mixed(metadata['NameEfficient'], metadata['weightsPath'], N_CLASSES)
    ...
    for epoch in range(EPOCHS):
        ...
        train_Mixed_distributed_NC(model, CLASS_NAMES, N_CLASSES, epoch, train_loader, train_len, optimizer, writer, device, rank)
        model.eval()
        AUROC_avg, loss = val_Mixed_distributed_NC(model, CLASS_NAMES, N_CLASSES, epoch, val_loader, val_len, optimizer, writer, device, rank)
        model.train()
        scheduler.step(loss)
        if rank == 0:
    ...
    if rank == 0:
        ...
        model.eval()
        test_NC_Mixed(metadata, model, test_loader, test_len, CLASS_NAMES, N_CLASSES, writer)
